...
def require_management(current_user: User = Depends(get_current_user)) -> User:
    """Permite el paso a Admin, Desarrolladores y Docentes para gestión."""
    if current_user.role not in ["admin", "developer", "docente"]:
        raise HTTPException(
            status_code=403, 
            detail="Privilegios de gestión requeridos (Admin, Developer o Docente)."
        )
    return current_user
