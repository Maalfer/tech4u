...
def generate_lab(data: LabGeneratorPayload, _: User = Depends(require_developer), db: Session = Depends(get_db)):
...
def get_lab_student(
    ...
    prev_lab = db.query(Lab).filter(
        Lab.module_id == lab.module_id,
        Lab.id < lab.id, 
        Lab.is_active == True
    ).order_by(Lab.id.desc()).first()
    
    if not prev_lab:
        lab.is_unlocked = True
    else:
        # Check if the current lab is one of the first two in its module
        # This mirrors the logic in list_labs_by_module for freemium
        ...
        try:
            lab_index = [l.id for l in all_labs_in_module].index(lab.id)
            if lab_index < 2:
                lab.is_unlocked = True
            else:
...
def get_all_labs_admin(_: User = Depends(require_developer), db: Session = Depends(get_db)):
...
def create_lab(lab_data: LabCreate, _: User = Depends(require_developer), db: Session = Depends(get_db)):
...
def update_lab(lab_id: int, lab_data: LabCreate, _: User = Depends(require_developer), db: Session = Depends(get_db)):
...
def delete_lab(lab_id: int, _: User = Depends(require_developer), db: Session = Depends(get_db)):
...
def toggle_lab_visibility(lab_id: int, _: User = Depends(require_developer), db: Session = Depends(get_db)):
    db_lab = db.query(Lab).filter(Lab.id == lab_id).first()
    if not db_lab: raise HTTPException(status_code=404)
...
def create_skill_path(path_data: SkillPathCreate, _: User = Depends(require_developer), db: Session = Depends(get_db)):
...
def update_skill_path(path_id: int, path_data: SkillPathCreate, _: User = Depends(require_developer), db: Session = Depends(get_db)):
...
def toggle_skill_path(path_id: int, _: User = Depends(require_developer), db: Session = Depends(get_db)):
    db_path = db.query(SkillPath).filter(SkillPath.id == path_id).first()
...
def delete_skill_path(path_id: int, _: User = Depends(require_developer), db: Session = Depends(get_db)):
...
def create_module(module_data: ModuleCreate, _: User = Depends(require_developer), db: Session = Depends(get_db)):
...
def update_module(module_id: int, module_data: ModuleCreate, _: User = Depends(require_developer), db: Session = Depends(get_db)):
...
def toggle_module_validation(module_id: int, _: User = Depends(require_developer), db: Session = Depends(get_db)):
    db_module = db.query(Module).filter(Module.id == module_id).first()
...
def toggle_module_visibility(module_id: int, _: User = Depends(require_developer), db: Session = Depends(get_db)):
    db_module = db.query(Module).filter(Module.id == module_id).first()
    if not db_module: raise HTTPException(status_code=404)
...
def delete_module(module_id: int, _: User = Depends(require_developer), db: Session = Depends(get_db)):
