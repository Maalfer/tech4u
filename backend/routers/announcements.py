...
from auth import get_current_user, require_admin, require_developer, require_management
...
def create_announcement(
    ...
    _: User = Depends(require_management),  # admin + developer
    db: Session = Depends(get_db)
):
    """(Gestión) Crea un anuncio que se mostrará a todos los alumnos como pop-up."""
...
def get_all_announcements_admin(
    ...
    """(Gestión) Lista todos los anuncios con sus IDs para gestión."""
...
def toggle_announcement(
    ...
    """(Gestión) Activa o desactiva un anuncio."""
...
def delete_announcement(
    ...
    """(Gestión) Elimina un anuncio permanentemente."""
    ann = db.query(Announcement).filter(Announcement.id == announcement_id).first()
    if not ann:
        raise HTTPException(status_code=404, detail="Anuncio no encontrado")
    # Remove read records
...
def get_all_suggestions(
    _: User = Depends(require_developer),  # admin + developer
    db: Session = Depends(get_db)
):
...
def approve_and_integrate_suggestion(
    ...
    """
    (Admin/Developer) Toma una sugerencia, la desarrolla como pregunta oficial y
