...
def apply_referral_code(
    ...
    if not re.match(r'^[A-Z0-9]{4,12}$', code):
        raise HTTPException(status_code=400, detail="Código de referido inválido")
...
def admin_referral_stats(
    ...
    referrer_ids = [row.referrer_id for row in top_referrers_raw]
    users_map = {u.id: u for u in db.query(User).filter(User.id.in_(referrer_ids)).all()}
    top_referrers = []
    for row in top_referrers_raw:
        u = users_map.get(row.referrer_id)
        if u:
