"""
seed_teoria_acl_guide.py
═══════════════════════════════════════════════════════════════════════════════
Inserta en la base de datos:
  1. El subject "Redes" (si no existe)
  2. El post "Guía completa: Creación de ACLs en redes Cisco"

Uso:
    cd /path/to/backend
    python scripts/seed_teoria_acl_guide.py
═══════════════════════════════════════════════════════════════════════════════
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# SLUG HELPER (igual que en routers/teoria.py)
# ─────────────────────────────────────────────────────────────────────────────

def make_slug(text: str, db=None, model=None) -> str:
...
# ─────────────────────────────────────────────────────────────────────────────
# MARKDOWN CONTENT — GUÍA COMPLETA ACLs
# ─────────────────────────────────────────────────────────────────────────────

MARKDOWN_CONTENT = r"""# Guía Completa: Creación y Gestión de ACLs en Redes Cisco
...
- [ ] Sé cómo **revertir** los cambios si hay un problema
"""
...
def seed():
    ...
    db = SessionLocal()
    try:
        # 1. Obtener o crear el subject "Redes"
        subject = db.query(TheorySubject).filter(TheorySubject.slug == "redes").first()
        if not subject:
            subject = TheorySubject(
                name="Redes",
                slug="redes",
                description="Fundamentos de redes, TCP/IP, OSI, subredes, enrutamiento, switching y seguridad.",
                icon="🌐",
                order_index=1,
            )
            db.add(subject)
            db.flush()
            print("✅  Subject 'Redes' creado.")
        else:
            print(f"ℹ️   Subject 'Redes' ya existe (id={subject.id}).")

        # 2. Comprobar si el post ya existe
        post_slug = "guia-creacion-acls-redes-cisco"
        existing_post = db.query(TheoryPost).filter(TheoryPost.slug == post_slug).first()
        if existing_post:
            print(f"ℹ️   El post con slug '{post_slug}' ya existe (id={existing_post.id}).")
            print("     Para actualizarlo, bórralo primero o modifica su contenido desde el admin.")
            return

        # 3. Crear el post
        post = TheoryPost(
            subject_id=subject.id,
            title="Guía completa: Creación de ACLs en redes Cisco",
            slug=post_slug,
        ...
        print(f"    Título : Guía completa: Creación de ACLs en redes Cisco")
        print(f"    Slug   : {post_slug}")
        print(f"    Subject: Redes")
        print(f"    URL    : /teoria/redes/{post_slug}\n")
...
if __name__ == "__main__":
    print("\n🔧  Insertando guía ACLs en sección Teoría → Redes...\n")
    seed()
