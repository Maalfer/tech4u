...
class UserRegister(BaseModel):
    ...
    role: Optional[str] = Field("alumno", pattern="^(alumno)$")
...
class UserRoleUpdate(BaseModel):
    role: str = Field(..., pattern="^(admin|developer|alumno)$")
...
class PaginatedQuestionOut(BaseModel):
...
class PayPalOrderCreate(BaseModel):
    ...
    # amount is accepted for backwards compatibility but IGNORED by the server
    # — server always uses the canonical price from PLANS dict (security measure)
