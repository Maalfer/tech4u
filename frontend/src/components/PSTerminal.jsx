...
function SubscriptionGate() {
    return (
        <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-8 text-center">
            ...
            <p className="text-sm text-slate-400 font-mono mb-1">Disponible en planes <span className="text-amber-400 font-bold">Trimestral</span> y <span className="text-amber-400 font-bold">Anual</span></p>
...
export default function PSTerminal({ step, onStepComplete, stepIndex = 0 }) {
    ...
    // ── Subscription check ─────────────────────────────────────────────────────
    const allowed = ['quarterly', 'annual'].includes(user?.subscription_type)
        || user?.role === 'admin'
        || user?.role === 'developer';
