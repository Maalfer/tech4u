                ...
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h1 className="text-4xl font-black text-white uppercase italic mb-2">
                            Gestión de <span className="text-[#39FF14]">Cursos</span>
                        </h1>
                        <p className="text-slate-400 font-mono text-sm max-w-2xl">
                            Añade cursos en vídeo, gestiona el temario y sube los enlaces de YouTube.
                        </p>
                    </div>
                </div>
                ...
                {view === 'list' && (
                    <>
                        <button
                            ...
                            className="mb-8 px-6 py-3 bg-[#39FF14] text-black font-black uppercase tracking-wider rounded-xl hover:scale-105 transition-transform flex items-center gap-2"
                        >
                ...
                {view === 'course_form' && (
                    <div className="max-w-2xl">
                        ...
                        <form onSubmit={handleSaveCourse} className="bg-[#111] border border-white/5 rounded-3xl p-8 space-y-6">
                            <div>
                                <label className="block text-[#39FF14] font-mono text-xs uppercase tracking-wider mb-2">Título del Curso</label>
                                <input required type="text" value={courseForm.title} onChange={e => setCourseForm({ ...courseForm, title: e.target.value })} className="w-full bg-[#050505] border border-white/10 rounded-xl px-4 py-3 text-white font-mono text-sm focus:border-[#39FF14] outline-none transition-colors" placeholder="Ej: Curso Avanzado Patrón Yates" />
                            </div>
                            <div>
                                <label className="block text-[#39FF14] font-mono text-xs uppercase tracking-wider mb-2">Descripción (Opcional)</label>
                                <textarea value={courseForm.description} onChange={e => setCourseForm({ ...courseForm, description: e.target.value })} className="w-full bg-[#050505] border border-white/10 rounded-xl px-4 py-3 text-white font-mono text-sm focus:border-[#39FF14] outline-none transition-colors h-32" placeholder="Resumen de lo que se aprenderá..."></textarea>
                            </div>
                            <div>
                                <label className="block text-[#39FF14] font-mono text-xs uppercase tracking-wider mb-2">URL Miniatura (Opcional)</label>
                                <input type="url" value={courseForm.thumbnail_url} onChange={e => setCourseForm({ ...courseForm, thumbnail_url: e.target.value })} className="w-full bg-[#050505] border border-white/10 rounded-xl px-4 py-3 text-white font-mono text-sm focus:border-[#39FF14] outline-none transition-colors" placeholder="https://ejemplo.com/imagen.jpg" />
                            </div>
                            <button type="submit" className="w-full py-4 bg-[#39FF14] text-black font-black uppercase tracking-wider rounded-xl hover:scale-105 transition-transform flex justify-center items-center gap-2">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                        ...
                        <div>
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <div>
                                    <p className="text-[#39FF14] font-mono text-[10px] uppercase">Gestionar Temario</p>
                                    <h2 className="text-2xl font-black text-white uppercase">{selectedCourse.title}</h2>
                                </div>
                            </div>

                            <form onSubmit={handleSaveLesson} className="bg-[#111] border border-[#39FF14]/30 rounded-3xl p-8 space-y-6">
                                ...
                                <div>
                                    ...
                                    <input required type="text" value={lessonForm.title} onChange={e => setLessonForm({ ...lessonForm, title: e.target.value })} className="w-full bg-[#050505] border border-white/10 rounded-xl px-4 py-3 text-white font-mono text-sm focus:border-[#39FF14] outline-none" placeholder="Ej: 1. Introducción a la Navegación" />
                                </div>
                                <div>
                                    <label className="block text-slate-400 font-mono text-xs uppercase tracking-wider mb-2">URL de YouTube</label>
                                    <input required type="url" value={lessonForm.youtube_url} onChange={e => setLessonForm({ ...lessonForm, youtube_url: e.target.value })} className="w-full bg-[#050505] border border-white/10 rounded-xl px-4 py-3 text-[#39FF14] font-mono text-sm focus:border-[#39FF14] outline-none" placeholder="https://www.youtube.com/watch?v=..." />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="col-span-2">
                                        ...
                                        <textarea value={lessonForm.description} onChange={e => setLessonForm({ ...lessonForm, description: e.target.value })} className="w-full bg-[#050505] border border-white/10 rounded-xl px-4 py-3 text-white font-mono text-sm focus:border-[#39FF14] outline-none h-24"></textarea>
                                    </div>
                                    <div className="col-span-1">
                                        <label className="block text-slate-400 font-mono text-xs uppercase tracking-wider mb-2">Orden (Index)</label>
                                        <input required type="number" min="0" value={lessonForm.order_index} onChange={e => setLessonForm({ ...lessonForm, order_index: parseInt(e.target.value) })} className="w-full bg-[#050505] border border-white/10 rounded-xl px-4 py-3 text-white font-mono text-sm focus:border-[#39FF14] outline-none" />
                                    </div>
                                </div>
                                <div className="flex gap-4 pt-4 border-t border-white/10">
                                    <button type="submit" className="flex-1 py-3 bg-[#39FF14] text-black font-black uppercase tracking-wider rounded-xl hover:scale-105 transition-transform flex justify-center items-center gap-2">
                        ...
                        <div className="bg-[#111] border border-white/5 rounded-3xl p-6 lg:h-[calc(100vh-12rem)] flex flex-col">
                            <h3 className="text-white font-black uppercase text-lg mb-4 flex items-center justify-between">
                                Temario Actual
                                <span className="text-[#39FF14] font-mono text-[10px] px-3 py-1 bg-[#39FF14]/10 rounded-full">{selectedCourse.lessons?.length || 0} Clases</span>
                            </h3>
                            <div className="flex-1 overflow-y-auto space-y-3 pr-2">
                                {selectedCourse.lessons?.length === 0 ? (
                                    ...
                                    [...(selectedCourse.lessons || [])].sort((a, b) => a.order_index - b.order_index).map((lesson, idx) => (
                                        <div key={lesson.id} className={`p-4 rounded-xl border flex items-center justify-between gap-4 transition-colors ${editingLessonId === lesson.id ? 'bg-[#39FF14]/5 border-[#39FF14]/50' : 'bg-black border-white/5 hover:border-white/20'}`}>
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <a href={lesson.youtube_url} target="_blank" rel="noreferrer" className="text-[#39FF14] hover:underline font-mono text-[10px] uppercase truncate inline-block max-w-[150px]">Link YouTube</a>
                                                </div>
                                            </div>
                                            <div className="flex gap-2">
